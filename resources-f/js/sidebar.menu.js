document.addEventListener('DOMContentLoaded', () => {
	...
	Array.prototype.forEach.call(elements, function(el) {
		...
		all.forEach((value, key) => {
			...
			// if(value.classList.contains('link-current')) {--for by default active
			// 	value.classList.add('active', 'down');
			// 	value.nextElementSibling.style.display = 'block';
			// }
			if(value.classList.contains('')) {
				value.classList.add('active', 'down');
				value.nextElementSibling.style.display = 'block';
			}
			value.addEventListener('click', (event) => {
				...
				!(value.classList.contains('link-current')) ? value.classList.add('link-current'): value.classList.remove('link-current');
			});
		});
	});
