...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function convertHTML($str) {
        return addslashes(htmlentities(htmlspecialchars($str, ENT_COMPAT)));
    }

    protected function decodeHTML($str) {
        return stripslashes(html_entity_decode(htmlspecialchars_decode($str)));
    }

    public function Authenticate($api_key) {
        if ($api_key <> "") {
            if(WOSA_API_KEY === $api_key)
            {
                return true;
            }
            else{
        ...
        return false;
           /*  $CI = & get_instance();
            $query = $CI->db->query("SELECT api_key FROM wosa_keys WHERE api_key = '" . $api_key . "' and active = 1 ");
            if ($query->num_rows() > 0) {
                return true;
            }
        }
    ...
    public function Authenticate_login2($mno,$countryCode) {
        if ($mno <> "") {
            $CI = & get_instance();
            $query = $CI->db->query("SELECT id FROM students WHERE mobile = '" . $mno . "' and country_code = '" . $countryCode . "' and active = 1");
            if ($query->num_rows() > 0) {
                return true;
            }
        }
    ...
    public function Authenticate_login($token,$mno) {
        if ($token <> "") {
            $CI = & get_instance();
            $query = $CI->db->query("SELECT id FROM students WHERE token = '" . $token . "' and mobile = '" . $mno . "' and active = 1");
            if ($query->num_rows() > 0) {
    ...
    public function _getorderTokens($length) {
        $key = '';
    ...
    function sendEmailTostd_complaints_($subject,$data){
        ...
        $this->email->to($data['email']);
        $this->email->subject($subject);
        $body = $this->load->view('emails/sendEmailTostd_complaints.php',$data,TRUE);
        $this->email->message($body);
        $this->email->send();
    } 

    function sendEmailTostdForsubscriptionOTP($subject,$data){
    ...
    function sendEmailTostdOnSubscription($subject,$data){
    ...
    public function sendEmailTostd_ApiBookingCreated($subject,$data)
    {
     ...
     $this->email->to($data['email']);
     $this->email->subject($subject);
     $body = $this->load->view('emails/sendEmail_SessionCreated.php',$data,TRUE);
     $this->email->message($body);
     $this->email->send();
  } 
  public function sendEmailTostd_Aptbooking_admin($subject,$data)
  {
   ...
   $this->email->to($data['email']);
   $this->email->subject($subject);
   $body = $this->load->view('emails/sendEmail_SessionBooking_admin.php',$data,TRUE);
   $this->email->message($body);
   $this->email->send();
}
 function sendEmailTostd_packsubs_($subject,$data){       
    ...
    function sendEmailTostd_packsubsfail_($subject, $data)
    {
        ...
        $this->email->to($data['email']);
        $this->email->subject($subject);
        $body = $this->load->view('emails/package_fail_subscribtion_email.php', $data, TRUE);
        $this->email->message($body);
        $this->email->send();
    }
    ...
    public function sendEmailTostd_Aptbooking($subject,$data)
    {
     ...
     $this->email->to($data['email']);
     $this->email->subject($subject);
     $body = $this->load->view('emails/sendEmail_sessionbooking.php',$data,TRUE);
     $this->email->message($body);
     $this->email->send();
 }
