    ...
    <div class="col-md-12">
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">Search Offline Pack</h3>
            </div>
            
            <?php echo form_open('adminController/package_master/offline_pack',array('method'=>'get')); ?>
                <div class="box-body">
                        ...
                        <?php $center_id_post = $this->input->get('center_id[]'); ?>
                            ...
                            <div class="form-group">
                                <select name="center_id[]" id="center_id" class="form-control selectpicker selectpicker-ui-100"  data-show-subtext="true" data-live-search="true" data-actions-box="true" multiple="multiple">
                                    ...
                                    foreach($all_branch as $p){
                                        $selected='';
                                        if(in_array($p['center_id'],$center_id_post)){
                                            $selected='selected="selected"';
                                        }
                ...
                <div class="col-md-12">
                    ...
                    <a href="<?php echo site_url('adminController/package_master/offline_pack'); ?>" class="btn btn-reset rd-20"> Reset</a>
                </div>
                </div>
            <?php echo form_close(); ?>
        </div>
        ...
        <div class="box">
            <div class="box-header bg-danger">
            	...
            	<div class="box-tools">
                    ...
                    ?>
                    <a href="<?php echo site_url('adminController/package_master/add_offline_pack'); ?>" class="btn btn-danger btn-sm">Add</a>
                    <?php } ?>
                     
                    <?php 
                        /*if($this->Role_model->_has_access_('package_master','offline_pack')){
                    ?>
                    <a href="<?php echo site_url('adminController/package_master/offline_pack'); ?>" class="btn btn-danger btn-sm">Offline Packages</a><?php } ?>
            ...
            <div class="box-body">
                ...
                <div  class="table-ui-scroller">
                    <div class="table-responsive mheight200">
                <table class="table table-striped table-bordered table-sm">
                    ...
                    <tr>
						...
						<td><?php echo $sr; ?></td>	
                        <td><?php echo PREFIX_INHOUSE_PACK_ID.$p['package_id']; ?></td>
                        <td>
                            <?php 
                                if($p['is_offline']==1){
                                    echo '<span class="text-muted">Offline Pack</span>';
                                }else{
                                    echo '<span class="text-success">Online Pack</span>';
                                } 
                            ?>                                
                        </td>
                        ...
                        <?php 
                            foreach ($p['Package_category'] as $pc) {
                                echo $category_name = $pc['category_name'].', ';
                            }
                        ?>                                                    
                        </td>
                        <td>
                        ...
                        <?php 
                            foreach ($p['Package_timing'] as $pt) {
                                echo $course_timing = $pt['course_timing'].', ';
                            }
                        ?>                                                    
                        </td>  
                        <td>
                        ...
                        <td>
                            ...
                            if($p['active']==1){
                                ...
                                echo '<span class="text-danger"><a href="javascript:void(0);" id='.$p['package_id'].' data-toggle="tooltip" title="Click to Activate" onclick=activate_deactivete('.$p['package_id'].','.$one.',"'.$table.'","'.$pk.'") >'.DEACTIVE.'</a></span>';
                            }
                            ?>                                
                        </td>
                        <td>
                            <?php 
                            if($p['publish']==1){
                                echo '<span class="text-success"><a href="javascript:void(0);">'.ACTIVE.'</a></span>';
                            }else{
                                echo '<span class="text-danger"><a href="javascript:void(0);">'.DEACTIVE.'</a></span>';
                            }
                            ?>                                
                        </td>
						<td>
                            <?php 
                                if($this->Role_model->_has_access_('package_master','edit_offline_pack_')){
                            ?>
                            <a href="<?php echo site_url('adminController/package_master/edit_offline_pack_/'.$p['package_id']); ?>" class="btn btn-info btn-xs" data-toggle="tooltip" title="Edit"><span class="fa fa-pencil"></span> </a> <?php } ?>
                            
                            <!-- <a href="<?php echo site_url('adminController/package_master/remove_offline_pack/'.$p['package_id']); ?>" class="btn btn-danger btn-xs" data-toggle="tooltip" title="Delete" onclick="return confirm('Are you sure you want to delete this item?');"><span class="fa fa-trash"></span> </a> -->
                        </td>
                    </tr>
                    <?php } ?>
...
$(document).ready(function(){
    ...
    $(document).on("click",".listCheckboxActionButton",function(){
        ...
        if(listRecords) {
            ...
            dataParameters["list_type"]             = "offline_pack";
