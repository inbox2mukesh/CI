...
$formType = isset($formType) && $formType ? $formType : "";
?>
<div class="qury-form-row">
  <div class="font-20 text-center mt-0 text-uppercase">
    <?php if ($formType == "workshop") { ?>
      <span class="text-theme-color-2 font-weight-600 text-red">Book</span> Now
    <?php } else { ?>
  ...
  <div class="form-group col-md-6 col-sm-6">
    ...
    <input type="text" name="fname" id="fname_qnform" class="fstinput form-control allow_alphabets length_validate" placeholder="First Name" value="<?php if (isset($this->session->userdata('student_login_data')->fname)) {
                                                                                                                                                      echo $this->session->userdata('student_login_data')->fname;
                                                                                                                                                    } else {
                                                                                                                                                      echo "";
                                                                                                                                                    } ?>" autocomplete="off" <?php echo $readOnly; ?> maxlength="30">
    <div class="valid-validation fname_qnform_err"></div>
  </div>
  <div class="form-group col-md-6 col-sm-6">
    <label>Last Name</label>
    <input type="text" name="lname" id="lname_qnform" class="fstinput form-control allow_alphabets" placeholder="Last Name" autocomplete="off" value="<?php if (isset($this->session->userdata('student_login_data')->lname)) {
                                                                                                                                                        echo $this->session->userdata('student_login_data')->lname;
                                                                                                                                                      } else {
                                                                                                                                                        echo "";
                                                                                                                                                      } ?>" maxlength="30" <?php echo $readOnly; ?> class="valid-validation lname_qnform_err">
  </div>
  <div class="form-group col-md-6 col-sm-6">
  ...
  <div class="form-group col-md-6 col-sm-6">
    ...
    <input type="tel" name="mobile" id="mobile_qnform" class="fstinput form-control allow_numeric" placeholder="Valid Phone No." maxlength="10" value="<?php if (isset($this->session->userdata('student_login_data')->mobile)) {
                                                                                                                                                          echo $this->session->userdata('student_login_data')->mobile;
                                                                                                                                                        } else {
                                                                                                                                                          echo "";
                                                                                                                                                        } ?>" <?php echo $readOnly; ?> autocomplete="off">
    <span class="valid-validation mobile_qnform_err"></span>
  </div>
  <div class="form-group col-md-6 col-sm-6">
    <label>Email ID<span class="text-red">*</span></label>
    <input type="email" name="email" id="email_qnform" class="fstinput form-control" placeholder="Valid Email ID" value="<?php if (isset($this->session->userdata('student_login_data')->email)) {
                                                                                                                            echo $this->session->userdata('student_login_data')->email;
                                                                                                                          } else {
                                                                                                                            echo "";
                                                                                                                          } ?>" maxlength="60" autocomplete="off" <?php echo $readOnly; ?>>
    <span class="valid-validation email_qnform_err"></span>
  </div>
  <div class="form-group col-md-6 col-sm-6">
    <label>DOB<span class="text-red">*</span></label>
    <div class="has-feedback">
      <input type="text" data-inputmask="'alias': 'date'" class="fstinput form-control dob_mask" name="dob" id="dob_qnform" placeholder="dd/mm/yyyy" value="<?php if (isset($this->session->userdata('student_login_data')->dob)) {
                                                                                                                                                              echo $this->session->userdata('student_login_data')->dob;
                                                                                                                                                            } else {
                                                                                                                                                              echo "";
                                                                                                                                                            } ?>" autocomplete="off" onchange="validatedob(this.value,this.id)" onkeyup="filldob(this.value);" ?php echo $readOnly;?>
  ...
  <?php } else { ?>
    <?php if ($formType == "workshop") { ?>
      <div class="form-group col-md-12">
        ...
        <select id="course_name" name="course_name" class="selectpicker form-control" aria-required="true" data-live-search="true">
          <option value="">Select Courses</option>
          <?php if (WORKSHOP_COURSES) { ?>
            <?php foreach (WORKSHOP_COURSES as $course) { ?>
              <option value="<?php echo $course; ?>"><?php echo $course; ?></option>
            <?php } ?>
          <?php } ?>
        </select>
        <span class="valid-validation course_name_err"></span>
      </div>
    <?php } else { ?>
      <div class="form-group col-md-12" <?php echo $hide; ?>>
        <label>Services<span class="text-red">*</span></label>
        <select class="form-control selectpicker" name="enquiry_purpose_id" id="enquiry_purpose_id">
          ...
          <?php if (!empty($serviceDataAll)) {
            foreach ($serviceDataAll->error_message->data  as $d) { ?>
  ...
  <div class="text-right col-md-12">
    ...
    <?php } else { ?>
      <?php $buttonText = "Send"; ?>
    <?php } ?>
    <button class="enqBtn btn btn-red" onclick="return validate_enq();"><?php echo $buttonText; ?></button>
...
<div class="reg-otp">
        ...
        <div class="modal-body">
          <div class="reg-modal clearfix"> <span class="cross-btn pull-right text-white hide-btn" data-dismiss="modal"><i class="fa fa-close font-20"></i></span>
            <div class="reg-otp-info text-center text-white ">
              ...
              <p class="mb-10 font-12">Verification Code has been sent on your <?php if (DEFAULT_COUNTRY == 101) {
                                                                                  echo "mobile";
                                                                                } else {
                                                                                  echo "email";
                                                                                } ?>.</p>
  ...
  <!-- <script src="<?php //echo site_url('resources-f/'); 
                    ?>js/date-mask.js"></script>
  ...
  <script type="text/javascript">
          ...
          if (response.status == 1) {
            ...
            <?php if (DEFAULT_COUNTRY == 101) { ?>
              gtag('event', 'conversion', {
                'send_to': 'AW-11071945767/rYbECPGWmp8YEKf4wZ8p'
              });
            <?php } ?>
            <?php
    ...
    function validate_enq() {
      ...
      <?php if ($formType == "workshop") { ?>
        ...
        if (courseName == "") {
          $(".course_name_err").text("Please choose course");
          flag = 0;
        }
      <?php } else { ?>
        message = $("#message_qnform").val();
