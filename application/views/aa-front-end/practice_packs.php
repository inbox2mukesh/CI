...
<span class="pull-right font-weight-600" id="down"><a href="">Clear All </a></span> </div> -->
      <!---EndFilter--->
    </div>
</section>
<!---Modal Info-->
...
<span> Offer Price: <ins><span class="font-weight-400 price">' . $p->discounted_amount . '</span></ins></span>';
    ...
    <!--END GRID CONTAINER-->
    <?php if ($AllPack_PP_count > LOAD_MORE_LIMIT_8) { ?>
...
<script>
  function loadmore() {
    ...
    $.ajax({
      url: "<?php echo site_url('practice_packs/ajax_loadmore_practicepack'); ?>",
      type: 'post',
      dataType: 'json',
      data: {
        offset: $('#offset').val(),
  ...
  function checkdobp(data, id) {
    ...
    if (isOver15(new Date(dt)) == false) {
      // $('.dob_mask_n').focus();
      // $('.dob_mask_n').val('');
      $(idd).html('You must have at least 15 years of age');
      return false;
    }
  }
...
<script type="text/javascript">
  ...
  function Getcategory() {
    ...
    if (test_module_id && programe_id) {
      $.ajax({
        url: "<?php echo site_url('practice_packs/Getcategory'); ?>",
        async: true,
        type: 'post',
        data: {
          test_module_id: test_module_id,
