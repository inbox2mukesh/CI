 ...
 *
 **/
class Test_preparation_material_topic extends MY_Controller{
    ...
    function ajax_check_freeresourcetopic_duplicacy(){
        ...
        if($topic_id and $topic) {
            echo $response= $this->Test_preparation_material_topic_model->check_topic_duplicacy2($topic,$topic_id);
        }else{
            echo $response=$this->Test_preparation_material_topic_model->check_topic_duplicacy($topic); 
        }
    }
    ...
    function index(){
        ...
        $config['total_rows'] = $this->Test_preparation_material_topic_model->get_all_topic_count();
        $this->pagination->initialize($config);
        $data['title'] = 'All Topic';
        $data['topic_master'] = $this->Test_preparation_material_topic_model->get_all_topic($params);
        $data['_view'] = 'test_preparation_material_topic/index';
    ...
    function add(){        
		...
		if($this->form_validation->run()){
            ...
            $idd = $this->Test_preparation_material_topic_model->add_topic($params);
            if($idd==1){
                //activity update start              
                    $activity_name= TEST_PREPARATION_TOPIC_ADD;
                ...
                if($this->Role_model->_has_access_('test_preparation_material_topic','index')){
                    redirect('adminController/test_preparation_material_topic/index');
                }else{
                    redirect('adminController/test_preparation_material_topic/add');
                }                
            }elseif($idd==2){
            ...
            $data['_view'] = 'test_preparation_material_topic/add';
            $this->load->view('layouts/main',$data);
        }        
    }
    ...
    function edit($topic_id){  
        ...
        $data['topic_master'] = $this->Test_preparation_material_topic_model->get_topic($topic_id);
        if(isset($data['topic_master']['topic_id'])){         
            ...
            $this->form_validation->set_rules('topic','Topic','required|trim|min_length[4]|max_length[100]');
             if($this->form_validation->run()){               
                ...
                $idd = $this->Test_preparation_material_topic_model->update_topic($topic_id,$params,$data['topic_master']['topic']);                
                if($idd==1){
                    //activity update start             
                        $activity_name= TEST_PREPARATION_TOPIC_UPDATE;
                    ...
                    if($this->Role_model->_has_access_('test_preparation_material_topic','index')){
                        ...
                        redirect('adminController/test_preparation_material_topic/edit/'.$topic_id);
                    }          
                    redirect('adminController/test_preparation_material_topic/index');
                }elseif($idd==2){
                    $this->session->set_flashdata('flsh_msg', DUP_MSG);           
                ...
                $data['_view'] = 'test_preparation_material_topic/edit';
                $this->load->view('layouts/main',$data);                
            }
        }
        else
