...
class Get_session_course extends REST_Controller {  
    ...
    { 
        if(!$this->Authenticate($this->input->get_request_header('API-KEY'))) {            
          ...
          $bData = $this->Counseling_session_model->getSessionCourse($arr);
          if(!empty($bData)){
            $data['error_message'] = [ "success" => 1, "message" => "success", "data"=> $bData, 'groupId'=>$aData];   
          }else{
            $data['error_message'] = [ "success" => 0, "message" => "No course found!", "data"=> $bData, 'groupId'=>$aData];     
          }        
