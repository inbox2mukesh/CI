...
if(function_exists('otp_send_verification_email')){
    ...
    {
        $tobeSend = [];
        $content = 'Thank you for your association with Western Overseas Immigration. To complete the verification process, please enter the following One-Time Password (OTP) in the verification field provided:</br>';
        if(!empty($otp)|| $otp != null)
        {
            $content .= 'OTP : '.$otp.'</br>';
        }
        $content .= 'Please note that this OTP is valid for '.OTP_VALIDATE_TIME.' minutes only. If you have not attempted to verify your account, please ignore this email.</br>';
        $content .= 'If you encounter any issues with the verification process or have any questions, please do not hesitate to reach out to our customer support team at '.SUPPORT_EMAIL.'.';
...
if(function_exists('enquiry_email')){
    ...
    {
        $tobeSend = [];
        $content = 'Thank you for contacting us regarding [enquiry topic]. We appreciate your interest in our products/services and are pleased to have the opportunity to assist you.</br>';
        $content .= 'Our team has received your enquiry and we will be reviewing it shortly. We strive to respond to all enquiries ASAP.';
        if(!empty($enquiryid)|| $enquiryid != null)
        {
            $content .= 'Please note the enquiry id for future reference.</br><b>Enquiry Id : '.$enquiryid.'</b></br>';
        }        
...
if(function_exists('admin_enquiry_reply_email')){
    ...
    function admin_enquiry_reply_email($enquiryid=null,$admin_reply=null)
    {
        ...
        $content .= 'After reviewing your enquiry, we have gathered the following details to assist you:</br>';
        if(!empty($enquiryid)|| $enquiryid != null)
        {
            $content .= '<b>Enquiry Id : '.$enquiryid.'</b></br>';
        } 
        if(!empty($admin_reply)|| ($admin_reply != null))
        {
            $content .= $admin_reply.'</br>';
        }        
